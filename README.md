...
> **macOS 지원** – Maven 빌드는 Apple Silicon과 Intel 맥을 자동 감지하며, `src/main/resources/MyApp.icns` 아이콘을 사용해 `.app` 번들을 함께 생성합니다.
...
`.app` 이미지는 같은 패키징 단계에서 `target/TubeMultiView.app`으로 생성됩니다. 별도의 아이콘을 넣을 필요 없이 `src/main/resources/MyApp.icns`가 자동으로 반영됩니다. 추가 배포 형식이 필요하면 `jpackage` 명령을 직접 실행해도 됩니다.
